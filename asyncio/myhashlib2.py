...
mytext='''Универсалы не так часто можно встретить на дорогах, кузов специфический, 
на любителя, большинство покупателей отдают предпочтение кроссоверам. 
Но вот Volvo V90 Cross Country из поколения в поколение пользуется стабильным спросом, 
смесь универсала и кроссовера пришлась многим по душе. Да и основной покупатель автомобилей 
Volvo - это человек в возрасте, хотя компания сейчас активно пытается привлечь молодую аудиторию, 
так что V90CC им отлично подходит. С новым же поколением Volvo V90 Cross Country 
автопроизводитель вполне может привлечь больше покупателей, 
автомобиль получился очень красивым, это отмечают даже те, кому универсалы никогда и не нравились. 
А главное, что на данный автомобиль обращает внимание молодежь, не раз слышал восхищенные отзывы о 
V90CC от данной аудитории, а значит, производитель попал в точку.
'''

h = hashlib.sha1() # or md5()
h.update(mytext.encode('utf-8'))
# print(h.digest()) # for binary output
print(h.hexdigest())
...
parser.add_argument(
    ...
    help='the name of hash algorithm to use',
)
parser.add_argument(
    'data',
    nargs='?',
